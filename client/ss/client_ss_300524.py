...
class MLClient(object):
    ...
    def process_task(self, node_id, task):
        ...
        if task.type == 'tensorflow':
            ...
            result = subprocess.run([sys.executable, 'tensorflow_task.py'], capture_output=True, text=True)
            model_path = result.stdout.strip()  # Correctly capture the model path
            if result.stderr:
                print("Error:", result.stderr)
            # Ensure the model path is correct and accessible

            try:
                with open('temp/saved_model/saved_model.pb', 'rb') as file:
                    model_data = file.read()
                print(model_data) # remove
                return model_data
            except IOError as e:
                print(f"Failed to read model data from {model_path}: {str(e)}")
                return None
        else:
