...
def tensorflow_train_model():
    model_dir = ensure_directory()  # Ensure the directory exists
    X_train, X_test, y_train, y_test = load_data()
    model = build_model()
    model.fit(X_train, y_train, epochs=50, verbose=0)
    loss, accuracy = model.evaluate(X_test, y_test, verbose=0)
    model_path = os.path.join(model_dir, 'saved_model')  # Ensure correct path
    tf.saved_model.save(model, model_path)
    return model_path

if __name__ == '__main__':
    model_path = tensorflow_train_model()
    print(model_path)  # This is the only output now
